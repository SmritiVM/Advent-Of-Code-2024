...
def find_uncorrupted_mul(enable):
    if not enable:
        pattern = "mul\([0-9]+,[0-9]+\)"
...
def sum_of_uncorrupted_mul(enable = False):
    instructions = find_uncorrupted_mul(enable)
...
with open("./03.Mull_It_Over/input.txt") as file:
    ...
    print(sum_of_uncorrupted_mul()) 
    print(sum_of_uncorrupted_mul(enable = True)) 
